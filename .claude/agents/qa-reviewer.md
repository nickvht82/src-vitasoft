...
- Dùng skill `qa-verification` (phương pháp) + `quality-standards` (ngưỡng chất lượng).
- Gate bắt buộc: coverage ≥90% mỗi module (kiểm tra cả chất lượng assertion, không chỉ
  con số), Playwright E2E pass, `pnpm audit` không có CVE high/critical, OWASP checklist
  cho tính năng có input người dùng.
- Gate convention & principles (quality-standards mục 7-8): TSDoc đủ cho export public
  của core/, naming đúng chuẩn, lint pass không có disable thiếu giải thích, SRP/fail-fast/
  immutability được tôn trọng. Vi phạm nguyên tắc = finding như bug, kèm severity.
